...
func NewAnalyzer(include []string, exclude []string) (*analysis.Analyzer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("prepare include patterns: %w", err)
	}

	a.exclude, err = newPatternsList(exclude)
	if err != nil {
		return nil, fmt.Errorf("prepare exclude patterns: %w", err)
	}

	return &analysis.Analyzer{
		Name:     "exhaustivenew",
		Doc:      "Checks if all structure fields are initialized in constructors",
		Run:      a.run,
...
func (a *analyzer) newFlagSet() flag.FlagSet {
	fs := flag.NewFlagSet("exhaustivenew flags", flag.PanicOnError)
...
func isNewFunc(fnc *ast.FuncDecl) bool {
	name := fnc.Name.String()
...
func (a *analyzer) newVisitor(pass *analysis.Pass) func(node ast.Node) {
	...
	return func(node ast.Node) {
		if fncLit, ok := node.(*ast.FuncDecl); ok {
			if isNewFunc(fncLit) {
				fnc = fncLit
			} else {
				fnc = nil
		...
		if len(missingFields) == 1 {
			pass.Reportf(node.Pos(), "%s is missing in new %s", missingFields[0], strctName)
		} else if len(missingFields) > 1 {
			pass.Reportf(node.Pos(), "%s are missing in new %s", strings.Join(missingFields, ", "), strctName)
		}
	}
}
...
func returnContainsLiteral(ret *ast.ReturnStmt, lit *ast.CompositeLit) bool {
	for _, result := range ret.Results {
		if l, ok := result.(*ast.CompositeLit); ok && lit == l {
			return true
		}
	}
